...
state_str = """  const [isTeamDropdownOpen, setIsTeamDropdownOpen] = useState(false)
...
"""
content = re.sub(r"  const \[isTeamDropdownOpen, setIsTeamDropdownOpen\] = useState\(false\)\n  const \[selectedTeam, setSelectedTeam\] = useState\(localStorage\.getItem\('syncmind_team'\) \|\| 'TEAM_OPS_ENG'\)\n", state_str, content)

# Add the UI section before the System Shutdown Scheduler
                    ...
                    <div className="flex flex-col gap-6">
                      <div className="flex items-start justify-between gap-4 p-4 rounded-xl border border-[var(--ws-card-border)] bg-[var(--hover)] shadow-inner">
                        ...
                        <button
                          ...
                          className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${swarmComputeEnabled ? 'bg-emerald-500/80' : 'bg-black/40 border border-white/10'}`}
                        >
...
content = re.sub(r"                  \{\/\* System Shutdown Section \*\/\}\s+<div className=\"w-full\">\s+<h3 className=\"text-lg font-semibold mb-1 opacity-90 text-red-500\">System Shutdown Scheduler<\/h3>", ui_str, content)
