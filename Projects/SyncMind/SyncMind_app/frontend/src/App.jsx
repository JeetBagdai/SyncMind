  ...
  async function newChat(ownerType) {
    ...
    try {
        ...
        const fresh = {
          ...
          messages: [],
          pinned: false,
          group: ownerType === 'TEAM' ? 'Team Workspace' : 'Personal Workspace',
          owner_id: owner
        }
  ...
  function deleteChat(id, e) {
    ...
    setConversations((prev) => {
      ...
      if (prev.length === 1 && prev[0].id === id) {
        ...
        setTimeout(() => newChat(deletedOwner?.startsWith('TEAM') ? 'TEAM' : 'PERSONAL'), 0);
                  ...
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex flex-col ${msg.role === 'user' ? 'items-end self-end' : 'items-start self-start'} max-w-[95%] sm:max-w-[85%] md:max-w-[80%]`}>
                        {msg.role === 'user' && msg.sender_name && (
                          <span className="text-[13px] text-[var(--txt)] bg-[var(--hover)] border border-[var(--ws-card-border)] rounded-full px-3 py-0.5 mb-2 font-bold tracking-wide shadow-sm">{msg.sender_name}</span>
                        )}
                        <div
        ...
        {activeTab === 'settings-view' && (
          <div className="ws-view">
            <section className="ws-panel">
              ...
              <div className="ws-body p-8 flex flex-col gap-10 text-[var(--txt)] max-w-5xl mx-auto w-full mt-2">
                {/* Profile Section */}
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        {isTeamDropdownOpen && (
                          <div className="absolute top-full left-0 w-full mt-2 bg-[var(--app-bg)] border border-[var(--ws-card-border)] rounded-lg shadow-2xl z-[100] overflow-hidden py-1">
                            {[
