                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button 
                          onClick={() => {
                            ...
                            .then(res => res.json())
                            .then(data => alert(data.message || "Shutdown scheduled."))
                            .catch(err => alert("Error scheduling shutdown: " + err));
                          }}
                          className="interactive-el bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30 px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                        >
                          Set Schedule
                        </button>
                      </div>
...
pattern = r"(<div className=\"ws-view\">\s*<section className=\"ws-panel\">\s*<header className=\"ws-head\">\s*<span className=\"ws-head-ic\">\s*<svg.*?Network Proof.*?</section>\s*</div>\s*)}"
...
if match:
    ...
    print("Could not find network-view to inject after")
